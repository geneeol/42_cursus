...
/*   Updated: 2023/01/31 05:13:14 by dahkang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init(t_philo **philos, t_args *args, t_rules *rules)
{
	...
	*philos = (t_philo *)malloc(sizeof(t_philo) * (rules->n_philo + 1));
	args->fork = (pthread_mutex_t *)
		malloc(sizeof(pthread_mutex_t) * (rules->n_philo + 1));
	if (!philos || !args->fork)
